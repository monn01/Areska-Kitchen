...
### Phase 13 — Konsistensi Tombol, Galeri Katalog di Landing Page & Gerbang Akun Wajib ✅ selesai 21 Juli 2026
- [x] **Bug akar ditemukan**: `cn()` (`lib/utils.ts`) cuma `clsx` murni tanpa resolusi konflik class Tailwind — override ukuran/padding lewat prop `className` (mis. `Button.tsx` yang punya `baseClasses` bawaan `px-6 py-3 text-base`) tidak selalu menang, tergantung urutan generate CSS, bukan urutan yang dimaksud kode. Ini sumber laporan user "tombol navbar ukurannya beda-beda". Fix: pasang `tailwind-merge`, `cn()` sekarang `twMerge(clsx(inputs))`
- [x] Tombol "Pesan Online"/"Chat WhatsApp" diseragamkan classNamenya di semua tempat (Navbar desktop & mobile, Hero, `StickyMobileCta`) — `StickyMobileCta` sebelumnya asimetris (WA icon-only kotak kecil vs "Pesan Online" full-width teks), sekarang keduanya `flex-1` sama besar dengan ikon+teks
- [x] `components/sections/MenuCta.tsx` diperkaya lagi: sampai 2 produk per kategori (maks 8 gambar, dari sebelumnya 1/kategori), tiap kartu dibungkus `TiltCard` untuk efek hover interaktif — permintaan user supaya galeri menu di landing page "lebih menarik dan membuat customer tertarik checkout"
- [x] **Gerbang akun wajib di `/katalog`** (perubahan keputusan dari "guest checkout + opsional akun" di Fase 2 awal — sekarang akses katalog online mewajibkan akun, guest checkout lama tidak disentuh/dihapus di level lain): `app/katalog/page.tsx` cek sesi server-side (`getServerSession`), kalau belum login konten katalog tetap dirender tapi di-`blur-sm` + `pointer-events-none`, ditumpuk `components/auth/KatalogAuthGate.tsx` (portal ke `document.body`, pola aman yang sama seperti `CartButton`) berisi form masuk/daftar; sukses auth memicu `router.refresh()` sehingga blur hilang tanpa pindah URL
- [x] `components/auth/AuthForms.tsx` (baru, dipakai bersama oleh `/account/login`, `/account/register`, dan overlay katalog) — tab Masuk/Daftar, tombol "Lanjutkan dengan Google" (OAuth), form Daftar sekarang phone-first ("daftar dengan WhatsApp": nama + no. WA wajib, email opsional untuk reset password), field ulangi-password, checkbox wajib setuju Kebijakan Layanan (link ke `/kebijakan-layanan`, bukan cuma teks kosong)
- [x] **Google OAuth** (`lib/auth.ts`): `GoogleProvider` ditambah tanpa `PrismaAdapter` (tetap JWT session, konsisten dengan setup lama) — `signIn` callback manual cari/`create` `User` berdasar email supaya tetap dapat `User.id` dari tabel sendiri. **Butuh input klien**: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` (gratis, dari Google Cloud Console) — kosong dulu di `.env`, tombol Google belum berfungsi sampai diisi (pola sama seperti kunci sandbox Midtrans)
- [x] **Login pakai email ATAU nomor WhatsApp**: `CredentialsProvider.authorize` sekarang cocokkan input ke `email` maupun `phone` (dinormalisasi via `normalizeIndonesianPhone` yang sudah ada) — field form tetap bernama "email" secara internal supaya `/admin/login` yang sudah ada tidak perlu diubah
- [x] **Lupa/reset password**: model baru `PasswordResetToken` (Prisma, token 32-byte random + expiry 30 menit), `app/account/forgot-password` & `app/account/reset-password` (halaman baru), `lib/mailer.ts` kirim email lewat Resend REST API kalau `RESEND_API_KEY` terisi — **kalau kosong, link reset cuma dicatat ke log server** (fallback dev, bukan gagal total, sama pola dengan Midtrans). **Butuh input klien**: daftar gratis di resend.com (s/d 3.000 email/bulan) lalu isi `RESEND_API_KEY` di `.env`. Catatan: reset password cuma jalan untuk akun yang punya email terdaftar (akun WA-only tidak bisa reset lewat email, disebutkan jelas di UI)
- [x] **Keputusan "WhatsApp" bukan OTP sungguhan**: sesuai pola proyek ini (sudah pernah tolak WhatsApp Business API berbayar untuk fitur #15), "Daftar dengan WhatsApp" diimplementasi sebagai nomor WA jadi identitas login (gratis, tanpa API pihak ketiga), BUKAN verifikasi kode OTP terkirim — itu butuh layanan berbayar (Fonnte/Twilio/Meta Cloud API) yang belum diputuskan. Kalau user mau OTP sungguhan nanti, ini perlu didiskusikan ulang (ada biaya berlangganan)
- [x] `prisma/schema.prisma`: `User.phone` sekarang `@unique` (dicek dulu tidak ada data dobel — cuma 1 user admin saat migrasi, aman), model baru `PasswordResetToken`. Migrasi `20260721070000_password_reset_and_unique_phone` diterapkan ke DB lokal
- [x] `app/kebijakan-layanan/page.tsx` (baru) — dokumen Syarat & Ketentuan + Kebijakan Privasi asli (bukan lorem ipsum), disesuaikan dengan praktik data proyek ini (data yang dikumpulkan, Midtrans sebagai pemroses pembayaran pihak ketiga, hak pengguna) — **catatan**: konten ditulis wajar oleh AI, bukan pengganti review notaris/legal sebelum go-live
- [x] `middleware.ts` — matcher `/account/((?!login|register).*)` ditambah pengecualian `forgot-password|reset-password` (kalau tidak, dua halaman publik baru ini ikut ter-redirect ke login, ditemukan & diperbaiki saat smoke test)
...
### Phase 18 — Navigasi Mobile (Bottom Tab Bar) & Perbaikan UX Mobile ✅ selesai 21 Juli 2026
Diskusi dulu sebelum dikerjakan (user tanya pendapat soal navigasi keluar dari alur checkout tanpa lewat profil): disepakati bottom tab bar mobile untuk halaman *browsing* saja, TIDAK di `/checkout` (jaga fokus, praktik umum e-commerce mengurangi distraksi saat bayar) dan TIDAK di halaman detail produk (digantikan bar "Tambah ke Keranjang" yang lebih relevan di titik itu, bukan navigasi).
- [x] **State drawer keranjang dipindah ke `CartContext`** (`isCartOpen`/`openCart`/`closeCart`) — sebelumnya local state di `CartButton.tsx`, sekarang di-share supaya tab bar mobile bisa buka drawer yang sama dari tempat lain
- [x] `components/ui/MobileTabBar.tsx` (baru) — 4 tab: Beranda, Katalog, Keranjang (buka drawer, bukan pindah halaman), Akun, dengan badge jumlah item di ikon keranjang. Dipasang di `/katalog`, `/account`, `/order/[id]` — TIDAK di `/checkout`, halaman detail produk, atau halaman auth (login/register/forgot/reset — biar fokus)
- [x] **Halaman detail produk** (`ProductDetail.tsx`) — 2 kritik user diperbaiki: (1) grid "Menu Lainnya" sebelumnya 1 kolom di mobile (hampir sama besar dengan gambar utama, terkesan seperti sedang memilih menu lain) → diubah jadi 2 kolom dari mobile; (2) tombol "Tambahkan ke Keranjang" sebelumnya di kolom sidebar yang di mobile letaknya di paling bawah (harus discroll dulu) → ditambah bar sticky terpisah khusus mobile (`fixed bottom-0 lg:hidden`) berisi stepper jumlah + tombol tambah, selalu terlihat tanpa scroll; kartu sidebar lama disembunyikan di mobile (`hidden lg:block`) supaya kontennya tidak dobel
...
- [ ] **Kunci Google OAuth** (`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` di `web/.env`) — buat gratis di console.cloud.google.com/apis/credentials (redirect URI: `<domain>/api/auth/callback/google`), wajib diisi sebelum tombol "Lanjutkan dengan Google" di login/daftar berfungsi
- [ ] **Kunci Resend** (`RESEND_API_KEY` di `web/.env`) — daftar gratis di resend.com (s/d 3.000 email/bulan), wajib diisi sebelum email reset password benar-benar terkirim ke pelanggan (sekarang link cuma tercatat di log server)
- [ ] Keputusan: apakah "Daftar dengan WhatsApp" perlu verifikasi OTP sungguhan (butuh langganan layanan pihak ketiga berbayar seperti Fonnte/Twilio/Meta Cloud API) atau tetap seperti sekarang (nomor WA sebagai identitas login, gratis, tanpa OTP) — lihat Phase 13
- [ ] Review dokumen `/kebijakan-layanan` (Syarat & Ketentuan + Privasi) oleh notaris/konsultan legal sebelum go-live — draf sekarang ditulis wajar oleh AI, belum ditinjau profesional hukum
...
### 21 Juli 2026 — Sesi 5 (Rombak landing page/e-commerce lanjutan, konsistensi tombol, gerbang akun wajib)

Lanjutan sesi arsitektur sebelumnya, revisi singkat dari user:

1. **Galeri gambar menu di landing page** — sesudah `MenuCta` (Sesi 4 lanjutan) sempat cuma 1 gambar/kategori, user minta lebih banyak gambar biar "lebih menarik". Dinaikkan jadi maks 2/kategori (8 total) + efek hover `TiltCard`.
2. **Tombol navbar/landing tidak konsisten ukurannya** — akar masalah ditemukan: `cn()` proyek ini cuma `clsx` polos, tidak resolve konflik class Tailwind yang bentrok (mis. `px-6` bawaan `Button.tsx` vs `px-5` dari override `className`), jadi ukuran menang berdasar urutan generate CSS, bukan urutan yang dimaksud — sumber semua laporan "tombol beda besar". Fix: pasang `tailwind-merge`. Ditambah audit manual: `StickyMobileCta` sebelumnya asimetris (WA icon-only vs teks full-width), disamakan jadi `flex-1` berdua.
3. **Rombak alur akun — gerbang wajib di `/katalog`**: klik "Pesan Online"/link katalog sekarang menampilkan katalog dalam kondisi blur + overlay form masuk/daftar (`KatalogAuthGate`, portal ke `document.body`), bukan bisa langsung diakses guest. Ini **mengubah keputusan Fase 2 awal** ("guest checkout + opsional akun") — cuma di titik masuk katalog, checkout sendiri belum diubah (lihat catatan "belum dikerjakan" di Phase 13).
4. **Daftar/masuk "kompleks" sesuai permintaan**: tombol Google OAuth (`GoogleProvider` tanpa `PrismaAdapter`, akun di-provision manual lewat `signIn` callback), login by email ATAU nomor WhatsApp, alur lupa/reset password lengkap (`PasswordResetToken` + `lib/mailer.ts` via Resend, fallback log kalau API key kosong), checkbox wajib setuju Kebijakan Layanan yang linknya ke dokumen sungguhan (`/kebijakan-layanan`, ditulis penuh bukan placeholder).
5. **Keputusan diambil sendiri tanpa nunggu konfirmasi user** (dan didokumentasikan supaya bisa dikoreksi kalau salah): "Daftar dengan WhatsApp" diimplementasi TANPA OTP sungguhan (nomor WA cuma jadi identitas login, gratis) — verifikasi OTP asli butuh API pihak ketiga berbayar yang belum pernah disepakati (konsisten dengan penolakan fitur #15 sebelumnya); provider email password-reset dipilih Resend (gratis, standar industri) dengan fallback log kalau belum dikonfigurasi. Kedua keputusan ini reversibel, tinggal ganti kalau user maunya lain.
