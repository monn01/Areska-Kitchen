...
export async function MenuCta() {
  ...
  // Satu produk representatif per kategori — cukup untuk preview visual di landing page
  // supaya pengunjung bisa lihat gambar menu tanpa bikin halaman penuh; katalog lengkap
  // & interaktif (semua produk, tambah ke keranjang) ada di /katalog.
  const preview = CATEGORY_ORDER.map((category) =>
      ...
      <div className="mx-auto max-w-container px-4 sm:px-6 lg:px-8">
        ...
        {preview.length > 0 && (
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {preview.map((item, i) => (
              <ScrollReveal key={item.id} variant="fade-up" delay={i * 0.08}>
